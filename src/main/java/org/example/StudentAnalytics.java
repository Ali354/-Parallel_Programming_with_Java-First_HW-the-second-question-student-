...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsImperative(
            final edu.coursera.parallel.Student[] studentArray) {
        List<edu.coursera.parallel.Student> activeStudents = new ArrayList<edu.coursera.parallel.Student>();

        for (edu.coursera.parallel.Student s : studentArray) {
            if (s.checkIsCurrent()) {
        ...
        double ageSum = 0.0;
        for (edu.coursera.parallel.Student s : activeStudents) {
            ageSum += s.getAge();
        }
     ...
     * @return Average age of enrolled students
     */
    public double averageAgeOfEnrolledStudentsParallelStream(final edu.coursera.parallel.Student[] studentArray) {
        Stream<edu.coursera.parallel.Student> activeStudents = Arrays.stream(studentArray).parallel()
                .filter(edu.coursera.parallel.Student::checkIsCurrent);

        return activeStudents.collect(Collectors.averagingDouble(edu.coursera.parallel.Student::getAge));
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsImperative(
            final edu.coursera.parallel.Student[] studentArray) {
        List<edu.coursera.parallel.Student> inactiveStudents = new ArrayList<edu.coursera.parallel.Student>();
        ...
        for (edu.coursera.parallel.Student s : inactiveStudents) {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        return Stream.of(studentArray)
                ...
                .collect(Collectors.groupingBy(edu.coursera.parallel.Student::getFirstName, Collectors.counting()))
                .entrySet().stream()
